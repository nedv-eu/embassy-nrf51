...
struct RtcDriver {
    ...
    period: Mutex<Cell<u32>>,//AtomicU32,
    alarm_count: Mutex<Cell<u8>>,
...
embassy::time_driver_impl!(static DRIVER: RtcDriver = RtcDriver {
    period: Mutex::const_new(CriticalSectionRawMutex::new(), Cell::new(0)),//AtomicU32::new(0),
    alarm_count: Mutex::const_new(CriticalSectionRawMutex::new(), Cell::new(0)),
...
impl RtcDriver {
    ...
    fn next_period(&self) {
        critical_section::with(|cs| {
            ...
            let period = self.period.lock(|period| {
                let p = period.get();
                let p = p+1; 
                period.replace(p);
                p
...
impl Driver for RtcDriver {
    fn now(&self) -> u64 {
        ...
        let period = self.period.lock(|period| {period.get()});
